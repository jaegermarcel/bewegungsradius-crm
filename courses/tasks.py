...
def send_course_start_email(course_id):
    ...
    try:
        ...
        if result.get("sent", 0) > 0:
            ...
            course.save()
            logger.info(
                f"✓ Start-E-Mail für Kurs {course_id} erfolgreich versendet und markiert"
            )
        ...
        return {"sent": 0, "errors": 1, "error": f"Kurs {course_id} nicht gefunden"}
...
def send_course_completion_email(course_id):
    ...
    try:
        ...
        if result.get("sent", 0) > 0:
            ...
            course.save()
            logger.info(
                f"✓ Abschluss-E-Mail für Kurs {course_id} erfolgreich versendet und markiert"
            )
