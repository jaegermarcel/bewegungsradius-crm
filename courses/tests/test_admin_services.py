...
class AttendanceListPDFGenerator:
    ...
    def generate(self, queryset):
        ...
        response['Content-Disposition'] = f'attachment; filename="anwesenheitsliste_{datetime.now().strftime("%Y%m%d")}.pdf"'
...
class AttendanceListCSVGenerator:
    ...
    def generate(self, queryset):
        ...
        response['Content-Disposition'] = f'attachment; filename="anwesenheitsliste_{datetime.now().strftime("%Y%m%d")}.csv"'
