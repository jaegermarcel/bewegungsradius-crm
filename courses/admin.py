...
from bewegungsradius.core.admin_styles import (
    DisplayHelpers, StatusIndicator,  # Verschiedene Helper
)
...
class CourseAdminForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['offer'].label_from_instance = lambda obj: f"{obj.title} ({obj.get_offer_type_display()})"
...
class LocationAdmin(LeafletGeoAdminMixin, ModelAdmin):
    ...
    def geocode_selected_locations(self, request, queryset):
        ...
        if count > 0:
            self.message_user(request, f"{count} Standort(e) geocodiert.", level='SUCCESS')
        if errors > 0:
            self.message_user(request, f"{errors} Standort(e) konnten nicht geocodiert werden.", level='WARNING')
        if count == 0 and errors == 0:
            self.message_user(request, "Alle ausgewählten Standorte haben bereits Koordinaten.", level='INFO')
...
class CourseAdmin(ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Termine & Ort', {
            'fields': (('start_date', 'end_date'), ('start_time', 'end_time'), ('location', 'is_weekly')),
            'description': 'Das Enddatum wird NICHT automatisch verlängert. '
                           'Bei Feiertagen an Kurstagen erhalten Sie eine Warnung.'
        }),
        ('Teilnehmer', {
            'fields': ('participants_inperson', 'participants_online')
        }),
        ('E-Mail Tracking', {
            'fields': (
    ...
    def start_time_display(self, obj):
        ...
        return DisplayHelpers.muted_text(start_time) if not obj.is_active else start_time
    ...
    def participants(self, obj):
        ...
        return DisplayHelpers.muted_text(participant) if not obj.is_active else participant
    ...
    def mark_as_inactive(self, request, queryset):
        ...
        self.message_user(
            request,
            f"{updated} Kurs(e) als inaktiv markiert.",
            level='WARNING'
        )
    ...
    def mark_as_active(self, request, queryset):
        ...
        self.message_user(
            request,
            f"{updated} Kurs(e) als aktiv markiert.",
            level='SUCCESS'
        )

    @action(description="Start E-Mail senden", url_path="send-email", permissions=["send_start_email_action"])
    def send_start_email_action(self, request, object_id):
        try:
            ...
            # Benachrichtigung
            self.message_user(request, f"✓ {result['sent']} Email(s) versendet", level='SUCCESS')
    ...
    @action(description="Ende E-Mail senden", url_path="send-completion-email", permissions=["send_end_email_action"])
