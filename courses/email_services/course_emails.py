...
class DiscountCodeService:
    ...
    def get_discount_code_for_participant(self, course, customer):
        ...
        try:
            discount_code = self.repository.find_active_for_course_and_customer(course, customer)

            if discount_code:
                logger.info(f"✓ Rabattcode gefunden: {discount_code.code}")
                return discount_code
            else:
                logger.debug(f"Kein Rabattcode für Kurs {course.id} und Kunde {customer.id}")
                return None
...
class CourseCompletionEmailService(BaseEmailService):
    ...
    def get_template_config(self, course, customer, **kwargs) -> EmailTemplateConfig:
        ...
        discount_code = self.discount_service.get_discount_code_for_participant(course, customer)
