...
class AccountingSummaryComponent(BaseComponent):
    ...
    def get_context_data(self, **kwargs):
        ...
        total_income = queryset.filter(entry_type='income').aggregate(Sum('amount'))['amount__sum'] or 0
        total_expense = queryset.filter(entry_type='expense').aggregate(Sum('amount'))['amount__sum'] or 0
...
class AccountingEntryAdmin(ModelAdmin):
    ...
    fieldsets = (
        ('Eintrag', {
            'fields': (('entry_type', 'date'), 'description', 'amount', )
        }),
        ('Rechnung', {
            'fields': ('invoice_link_display',)
        }),
        ('Notizen', {
            'fields': ('notes',)
        }),
    ...
    def amount_display(self, obj):
        ...
        if obj.entry_type == 'income':
            return format_html(f'<span style="color: #10b981; font-weight: 600;">+{obj.amount}€</span>')
        return format_html(f'<span style="color: #ef4444; font-weight: 600;">-{obj.amount}€</span>')
