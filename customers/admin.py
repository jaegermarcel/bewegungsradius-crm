...
from bewegungsradius.core.admin_styles import (
    Colors,  # Zentrale Farben
    StatusIndicator,  # Pre-made Status (3 Varianten)
    DisplayHelpers, SimpleText,  # Verschiedene Helper
)
from courses.models import Course

from .address_validator import CustomerAdminValidationMixin
from .models import Customer, CustomerDiscountCode, ContactChannel
...
class DiscountCodeEmailService:
    ...
    def _render_template(self, customer, discount_code):
        ...
        return render_to_string('email/notifications/discount_notification.html', {
            'customer': customer,
...
class CustomerAdmin(CustomerAdminValidationMixin, LeafletGeoAdminMixin, ModelAdmin):
    ...
    search_fields = ['display_as_two_line_heading', 'last_name', 'email', 'mobile', 'city', 'postal_code']
    list_filter = ['is_active', 'city', 'contact_channel', 'created_at', ('birthday', admin.EmptyFieldListFilter)]
    actions = ['archive_customers', 'unarchive_customers', 'geocode_selected_customers', 'export_to_csv']
    list_per_page = 50

    fieldsets = (
        ('Status', {'fields': (('is_active', 'archived_at'),)}),
        ('Persönliche Daten', {'fields': (('first_name', 'last_name'), ('email', 'mobile'), 'birthday')}),
        ('Kontakt', {'fields': ('contact_channel',)}),
        ('Adress-Validierung', {'fields': ('address_validation_display',), 'classes': ('wide',)}),
        ('Adresse', {'fields': (('street', 'house_number'), ('postal_code', 'city'), 'country')}),
        ('Geografische Position', {'fields': ('coordinates',), 'classes': ('collapse',)}),
    ...
    def age(self, obj):
        ...
        from datetime import date
        today = date.today()
        calculated_age = today.year - obj.birthday.year - (
...
class CustomerDiscountCodeAdmin(ModelAdmin):
    ...
    search_fields = ['code', 'customer__first_name', 'customer__last_name', 'customer__email', 'description', 'course']
    readonly_fields = ['created_at', 'used_at', 'created_by', 'email_sent_at', 'cancelled_at']
    actions = ['send_discount_code_email', 'mark_as_used', 'mark_as_cancelled', 'mark_as_planned']
    list_per_page = 50
    ...
    fieldsets = (
        ...
        ('Historie', {'fields': (('email_sent_at', 'used_at', 'cancelled_at'), 'cancelled_reason'), 'classes': ('collapse',)}),
        ('System', {'fields': (('created_at', 'created_by'),), 'classes': ('collapse',)}),
    )
    ...
    @action(description="Code via E-Mail versenden", url_path="send-email", permissions=["send_email_action"])
    def send_email_action(self, request, object_id):
        ...
        return redirect(reverse('admin:customers_customerdiscountcode_change', args=[object_id]))
    ...
    def send_discount_code_email(self, request, queryset):
        ...
        self.message_user(request, f"✅ {success} E-Mail(s) versendet.", level='SUCCESS')
        if errors:
            error_msg = "\n".join([f"{name}: {err}" for name, err in errors[:5]])
            self.message_user(request, f"❌ {len(errors)} Fehler:\n{error_msg}", level='ERROR')
    ...
    def mark_as_used(self, request, queryset):
        ...
        if not codes_to_use.exists():
            self.message_user(request, "⚠️ Alle sind bereits eingelöst.", level='WARNING')
            return
        for code in codes_to_use:
            code.use_code()
        self.message_user(request, f"✅ {codes_to_use.count()} Code(s) als eingelöst markiert.", level='SUCCESS')
    ...
    def mark_as_cancelled(self, request, queryset):
        ...
        if not codes_to_cancel.exists():
            self.message_user(request, "⚠️ Alle sind bereits storniert.", level='WARNING')
            return
        codes_to_cancel.update(status='cancelled', cancelled_at=timezone.now(), cancelled_reason="Manuell storniert")
        self.message_user(request, f"✅ {codes_to_cancel.count()} Code(s) storniert.", level='SUCCESS')
    ...
    def mark_as_planned(self, request, queryset):
        ...
        if not codes_to_reactivate.exists():
            ...
            return
        codes_to_reactivate.update(status='planned', cancelled_at=None, cancelled_reason="")
        self.message_user(request, f"✅ {codes_to_reactivate.count()} Code(s) reaktiviert.", level='SUCCESS')
