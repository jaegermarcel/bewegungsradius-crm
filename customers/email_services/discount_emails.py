...
class DiscountCodeEmailService(BaseEmailService):
    ...
    def get_template_config(
        self, customer, discount_code, **kwargs
    ) -> EmailTemplateConfig:
    ...
    def build_email_payload(
        self, customer, discount_code, html_content: str = None, **kwargs
    ) -> EmailPayload:
        ...
        for code in discount_codes:
            try:
                ...
                result["failed"].append(
                    {"customer": code.customer.get_full_name(), "error": str(e)}
                )
        ...
        for course in courses:
            ...
            for customer in participants:
                code = CustomerDiscountCode.objects.filter(
                    customer=customer, course=course, status="planned"
                ).first()
