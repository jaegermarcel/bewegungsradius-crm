...
from customers.models import Customer, CustomerDiscountCode, ContactChannel
...
def contact_channels(db):
    ...
    for name, slug in channel_data:
        channels.append(ContactChannel.objects.create(
            name=name,
            slug=slug,
            is_active=True
        ))
...
class TestCustomerModel:
    ...
    def test_customer_contact_channel_set_null_on_delete(self, customer, contact_channel):
...
class TestDiscountCodeValidation:
    ...
    def test_is_valid_calls_validator(self, mock_validator_class, planned_discount_code):
...
class TestCustomerIntegration:
    ...
    def test_customer_with_all_fields_populated(self):
        ...
        assert customer.get_full_address() == 'Hauptstraße 42, 80801 München, Deutschland'
