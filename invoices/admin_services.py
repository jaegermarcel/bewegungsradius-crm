...
class InvoiceActionHandler:
    ...
    def mark_as_sent(request, object_id, admin_instance):
        ...
        try:
            ...
            admin_instance.message_user(request, 'Rechnung nicht gefunden.', level='error')
        except Exception as e:
            admin_instance.message_user(request, f'Fehler beim Aktualisieren: {str(e)}', level='error')
            logger.error(f"Error marking invoice {object_id} as sent: {e}")

        return redirect(reverse_lazy('admin:invoices_invoice_change', args=(object_id,)))
    ...
    def stornieren(request, object_id, admin_instance):
        ...
        try:
            ...
            if invoice.status == 'cancelled':
                admin_instance.message_user(
                    request,
                    'Diese Rechnung ist bereits storniert.',
                    level='warning'
                )
                return redirect(reverse_lazy('admin:invoices_invoice_change', args=(object_id,)))
            ...
            admin_instance.message_user(request, f'Fehler beim Stornieren: {str(e)}', level='error')
    ...
    def download_storno_pdf(request, object_id, admin_instance):
        ...
        try:
            ...
            if invoice.status != 'cancelled':
                admin_instance.message_user(
                    request,
                    'Diese Rechnung ist nicht storniert.',
                    level='warning'
                )
                return redirect(reverse_lazy('admin:invoices_invoice_change', args=(object_id,)))
            ...
            try:
                ...
                logger.error(f"Storno PDF generation failed: {gen_error}", exc_info=True)
                admin_instance.message_user(
                    request,
            ...
            admin_instance.message_user(
                request,
                f'✓ Storno-PDF heruntergeladen.',
                level='success'
            )

            logger.info(f"Cancellation PDF downloaded: {filename} ({len(pdf_bytes)} bytes)")
            return response
