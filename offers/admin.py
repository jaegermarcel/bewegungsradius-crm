...
class ZPPCertificationAdmin(ModelAdmin):
    ...
    list_display = ['display_as_two_line_heading', 'format_display', 'validity_display', 'days_until_expiry_display', 'is_active_display']
    ...
    def format_display(self, obj):
        ...
        return DisplayHelpers.muted_text(format_text) if not obj.is_active else format_text
    ...
    def validity_display(self, obj):
        ...
        if days <= 0:
            ...
            return DisplayHelpers.muted_text(error_text) if not obj.is_active else error_text
        elif days <= 30:
            warning_text = SimpleText.literal_text(f'⚠️ {days} Tage', 'warning')
            return DisplayHelpers.muted_text(warning_text) if not obj.is_active else warning_text
        else:
            return DisplayHelpers.muted_text(f'{obj.valid_until.strftime("%d.%m.%Y")}') if not obj.is_active else f'{obj.valid_until.strftime("%d.%m.%Y")}'
    ...
    def days_until_expiry_display(self, obj):
        ...
        return DisplayHelpers.muted_text(f'{days} Tage') if not obj.is_active else f'{days} Tage'
...
class OfferAdmin(ModelAdmin):
    ...
    fieldsets = (
        ...
        # Preise
        ('Preise', {
            'fields': ('amount', 'tax_rate', 'is_tax_exempt', 'tax_amount', 'total_amount')
        }),
    ...
    def price_display(self, obj):
        ...
        if obj.offer_type == 'ticket_10':
            ...
            return DisplayHelpers.muted_text(f'{obj.total_amount}€ ({price_per_session}€/Einheit)') if not obj.is_active else f'{obj.total_amount}€ ({price_per_session}€/Einheit)'

        else:
            return DisplayHelpers.muted_text(
                f'{obj.total_amount}€') if not obj.is_active else f'{obj.total_amount}€'
    ...
    def description_display(self, obj):
        ...
        return DisplayHelpers.muted_text(description) if not obj.is_active else description
